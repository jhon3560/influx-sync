...
- 去重：`seq ≤ last_seq` 直接 0xff（last_seq 按连续前缀推进，V1.4.1）
- seq 从 1 开始（seq=0 保留给心跳；data seq=0 会被首帧去重吞掉，N7 文档化）
- 小缺口（重启后 last_seq 文件节流落后）：新连接首帧（= sender WAL 头）闭合（V1.4.2/N6）
- seq 大跳跃（>100000）：接受处理（幂等覆盖安全）——拒绝会造成停等死锁（V1.3.1）
