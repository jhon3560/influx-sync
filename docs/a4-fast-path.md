...
## 3. 启用时机（V1.7 起：打开即生效，与历史回填并行）

**V1.7 语义变更**：快路径不再等待游标追平——`fast_path.listen` 配置后**立即透传**
（`mode: off` 除外），实时同步"打开就生效"；历史回填由 `sync.backfill` 选项独立控制
（见 configuration.md §5.4），二者**并行互不阻塞**。V1.5/V1.6 的"追平后自动启用"
门控（activate_age/deactivate_age）已移除，旧配置文件中残留的这两项会被忽略。

- `mode`：`off`（仅信号，= 纯轮询行为）/ `on` / `auto`（**≡ on**，保留别名兼容旧配置）。
- 回填期间快路径与轮询的去重集照旧工作；轮询爬行前沿离"现在"很远时，去重集
  （保留 ~15s）对快路径数据的抑制会过期 → **回填追平末尾会重复转发 ~水位窗口的数据**
  （目标库幂等，计数不重复，带宽浪费可忽略）。
- 目标库观测提示：回填期间"新数据已到、历史还在补"——`sync_e2e_delay_seconds` 显示
  0~1s（实时新鲜），总条数缓慢增长；回填进度看 `sync_delay_seconds`。
