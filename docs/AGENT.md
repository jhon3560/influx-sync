...
- **工作文档必须入仓**：每轮工作成果、审计结论、修复记录一律写入 `docs/` 并提交
  （命名带版本/日期，如 `audit-v172-2026-08.md`），不要只留在对话里；
- **接手前先读目录**：开始新工作前先 `git log --oneline` + 读 docs/ 下的最新
  审计/修复文档，以上一轮落档结论为交接面；
- **审计报告固定格式**：结论先行 → 逐项核验表 → 残余缺陷（含取证记录）→ 修复建议；
- 临时取证/复现测试用完即删，结论以文档为准。
