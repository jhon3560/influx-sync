...
5. **历史回填（V1.7，随时可开关）**：`sync.backfill: all`（默认，全量）/`0`（仅实时）/
   `30d`（有界，支持 d 单位）。改配置+重启即生效，**不清数据目录**；只在值变化时回拨
   一次游标，不变重启不重发。**存量部署升级 V1.7 后默认 all 不会触发全库重发**
   （旧进度自动识别，只记录配置值）——想补历史改一次配置即可。库内数据比回拨边界晚时
   自动定位最早数据、跳过真空区。详见 [configuration.md](configuration.md) §5.4。
